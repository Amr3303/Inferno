...
//         console.log("setCurrentBroadcastId:", currentBroadcastId);
//         console.log("storedBroadcastId", storedBroadcastId);
//       }
//     };
...
//       fetchProcessMessages(selectedBroadcastId, token);
//     }
//   };
...
//   const { messagesType, loadingType, fetchMessagesType } = useFetchMessagesType(); // استخدام الـ hook
//   const [messageType, setMessageType] = useState<string>('text'); // مثال لاختيار النوع
...
//   useEffect(() => {
//     // جلب الرسائل من نوع معين عند تحميل الصفحة
//     fetchMessagesType(messageType);
//   }, [fetchMessagesType, messageType]); // سيتم التحديث عندما يتغير messageType
...
//   useEffect(() => {
//     const token = localStorage.getItem('token');
//     if (!token) return; // تأكد من وجود التوكن قبل أي طلب
...
//     }
//   }, [currentView]); // استمع للتغييرات على currentView و token
//   // Fetch specific message type when view changes
//   // useEffect(() => {
//   //   const token = localStorage.getItem('token');
...
//   useEffect(() => {
//     const token = localStorage.getItem('token'); // ضع هنا التوكن الصحيح
//     fetchMessages();
//   }, [fetchMessages]);
//   // Fetch all messages
...
//   // Shorthand functions for fetching specific message types
//   const fetchTextMessages = () => fetchMessagesType('text');
//   const fetchLocationMessages = () => fetchMessagesType('location');
//   const fetchProcessMessages = () => fetchMessagesType('process');
//   const fetchQueryMessages = () => fetchMessagesType('query');
